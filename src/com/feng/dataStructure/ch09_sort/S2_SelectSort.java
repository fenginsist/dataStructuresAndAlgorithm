 ...
 *
 *    冒泡是 比较后，将大值 放到最后，而 选择 是 假定第一个为最小值，比较后，将最小值放到数组第一个位置，以此类推。
 *
    ...
    public static int[] selectSort(int[] array) {
        ...
        for (int i = 0; i < array.length - 1; i++) {
            ...
            for (int j = i + 1; j < array.length; j++) {
                if (min > array[j]) {  // 说明假定的最小值，并不是最小。  找到最小的值 ，并保存到 minIndex  和 min 。
                    minIndex = j; // 重置 min
            ...
            if (minIndex != i) {
                array[minIndex] = array[i]; // 将 前面要 放的数据 赋值 给 后面的最小值的地方。
                array[i] = min;  // 将最小值 赋值到前面。
            }
//            System.out.println(Arrays.toString(array));
        }
        return array;
